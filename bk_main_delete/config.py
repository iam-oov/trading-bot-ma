...
def load_constants_module(module_suffix='base'):
    ...
    try:
        ...
        print(
            f"Warning: Constants module '{module_name}' not found. Using 'constants.base'.")
        ...
        try:
            ...
            print("CRITICAL Error: Could not import base constants module 'constants.base'. Verify it exists.", file=sys.stderr)
            print(f"Details: {e}", file=sys.stderr)
            sys.exit(1)
        except Exception as e:
            print(
                f"CRITICAL Error: Unexpected error loading constants module '{module_name}'.", file=sys.stderr)
            print(f"Details: {e}", file=sys.stderr)
            sys.exit(1)
...
def parse_arguments():
    ...
    if args:
        ...
        try:
            temp_constants = importlib.import_module(
                temp_constants_module_name)
                ...
                for i, arg in enumerate(args):
                    if i < len(keys):
                        ...
                        try:
                            ...
                            print(
                                f"TRADING parameter override: {key} = {overrides[key]}")
                            ...
                            if not modified_trading_params and custom_log_prefix is None and not is_dev:
                                ...
                                print(
                                    f"Argument '{arg}' ignored (expected number for override or prefix already set).", file=sys.stderr)
                        except KeyError:
                            # This shouldn't happen if keys are from temp_constants.TRADING
                            print(
                                f"Warning: Key '{key}' (from base) somehow not found for override.", file=sys.stderr)
                    else:
                        # If we haven't processed overrides yet AND haven't set a custom prefix,
                        ...
                        if not modified_trading_params and custom_log_prefix is None and not is_dev:
                            ...
                            print(
                                f"Extra argument '{arg}' ignored.", file=sys.stderr)

            else:
                print(
                    f"Warning: {temp_constants_module_name}.TRADING not found or not a dictionary. Cannot process overrides.", file=sys.stderr)
                # Treat remaining args as potential prefix if none set
                if args and custom_log_prefix is None and not is_dev:
                    custom_log_prefix = args[0]
                    print(
                        f"Using custom log prefix (fallback): {custom_log_prefix}")

        except ImportError:
            print(
                f"Warning: Could not import '{temp_constants_module_name}' to get override keys.", file=sys.stderr)
            # Treat remaining args as potential prefix if none set
            if args and custom_log_prefix is None and not is_dev:
                custom_log_prefix = args[0]
                print(
...
if TRADING_OVERRIDES and hasattr(CONSTANTS, 'TRADING') and isinstance(CONSTANTS.TRADING, dict):
    ...
    for key, value in TRADING_OVERRIDES.items():
        if key in CONSTANTS.TRADING:
            ...
            print(
                f"  Warning: Override key '{key}' not found in loaded constants '{MODULE_NAME}'.", file=sys.stderr)
elif TRADING_OVERRIDES:
    print(
        f"Warning: Overrides parsed, but CONSTANTS.TRADING not found or not a dict in '{MODULE_NAME}'. Overrides ignored.", file=sys.stderr)


# Define Log Path using the final LOG_PREFIX
LOG_PATH = os.path.join('log', LOG_PREFIX, PIN.replace(
    '/', os.sep))  # Ensure OS-agnostic path
...
NOTIFICATIONS_ACTIVE = getattr(
    CONSTANTS, 'NOTIFICATIONS', {}).get('ACTIVE', False)
NOTIFICATION_TIMEOUT = getattr(
    CONSTANTS, 'CLOSE_NOTIFICATION_TIMEOUT', 10)  # Default 10s
...
if not isinstance(TRADING_PARAMS, dict):
    print(
        f"Warning: CONSTANTS.TRADING in '{MODULE_NAME}' is not a dictionary. Using defaults.", file=sys.stderr)
...
for key, default in DEFAULT_TRADING_PARAMS.items():
    if key not in TRADING_PARAMS:
        print(
            f"Warning: Key '{key}' missing in {MODULE_NAME}.TRADING. Using default: {default}", file=sys.stderr)
...
ACTIVE_LOG = getattr(CONSTANTS, 'ACTIVE_LOG', True)
...
IN_PROGRESS_NAME = getattr(CONSTANTS, 'IN_PROGRESS',
                           {}).get('name', 'IN_PROGRESS')
FAST_SHORT_NAME = getattr(CONSTANTS, 'FAST_SHORT', {}
                          ).get('name', 'FAST_SHORT')
...
TYPE_DEFINITIONS = {
    ...
    FAST_SHORT_NAME: getattr(CONSTANTS, 'FAST_SHORT', {
                             'name': FAST_SHORT_NAME, 'emoji': '⚡️'})
}
# Ensure the dictionaries themselves exist if accessed directly elsewhere
if not hasattr(CONSTANTS, 'LONG'):
    CONSTANTS.LONG = TYPE_DEFINITIONS[LONG_NAME]
if not hasattr(CONSTANTS, 'SHORT'):
    CONSTANTS.SHORT = TYPE_DEFINITIONS[SHORT_NAME]
if not hasattr(CONSTANTS, 'FAST_SHORT'):
    CONSTANTS.FAST_SHORT = TYPE_DEFINITIONS[FAST_SHORT_NAME]
