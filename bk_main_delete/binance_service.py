...
class BinanceService:
    ...
    def __init__(self, api_key=None, api_secret=None, tld='com'):
        ...
        try:
            ...
            logger.log_message(
                "Binance client initialized and connected.", "GREEN")
        except Exception as e:
            logger.log_message(
                f"CRITICAL Error initializing Binance client: {e}", "RED")
    ...
    def get_usdt_futures_symbols(self):
        ...
        if not self.is_connected():
            logger.log_message(
                "Binance client not available (get_usdt_futures_symbols).", "RED")
            return []
        try:
            tickers = self.client.futures_symbol_ticker()
            symbols = [tick['symbol']
                       for tick in tickers if tick['symbol'].endswith('USDT')]
            print("🚀 ~ symbols:", symbols)
            return symbols
        except Exception as e:
            logger.log_message(
                f"Error getting symbols from Binance: {e}", "RED")
            return []
    ...
    def get_futures_klines(self, symbol, interval=Client.KLINE_INTERVAL_1MINUTE, limit=30):
        ...
        if not self.is_connected():
            logger.log_message(
                f"Binance client not available (get_futures_klines for {symbol}).", "RED")
            return None
        try:
            return self.client.futures_klines(symbol=symbol, interval=interval, limit=limit)
        except Exception as e:
            logger.log_message(
                f"Error getting klines for {symbol}: {e}", "RED")
            return None
    ...
    def get_futures_ticker_info(self, symbol):
        ...
        if not self.is_connected():
            logger.log_message(
                f"Binance client not available (get_futures_ticker_info for {symbol}).", "RED")
            return None
        try:
            return self.client.futures_ticker(symbol=symbol)
        except Exception as e:
            logger.log_message(
                f"Error getting ticker info for {symbol}: {e}", "RED")
            return None
