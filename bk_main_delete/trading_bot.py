...
try:
    ...
    print(
        f"CRITICAL ERROR: Failed to import core modules: {e}", file=sys.stderr)
...
def connect():
    ...
    logger.log_message(
        f'Trading Bot (PIN: {config.PIN}) connected to server.', 'GREEN')
    send_log_to_server(
        f'Trading Bot (PIN: {config.PIN}) connected to server.', 'GREEN')
...
def send_log_to_server(message, color="default"):
    ...
    if connected_to_server:
        try:
            ...
            sio_client.emit('log_from_script', {
                            'message': message, 'color': web_color_style})
        except socketio.exceptions.BadNamespaceError:
            logger.log_message(
                "Socket.IO connection lost (BadNamespaceError), attempting reconnect implicitly.", "RED")
...
def send_stats_to_server():
    ...
    try:
        in_progress_count = sum(results.get(
            config.IN_PROGRESS_NAME, {}).values())
        ...
        efficiency = round(winning_count * 100 / total_finished,
                           2) if total_finished > 0 else 0.0
        ...
        logger.log_message(
            "Socket.IO connection lost (BadNamespaceError) while sending stats.", "RED")
        connected_to_server = False
    except Exception as e:
        logger.log_message(
            f"Error calculating or sending statistics: {e}", "RED")
...
def send_active_operations_to_server():
    ...
    try:
        ...
        logger.log_message(
            "Socket.IO connection lost (BadNamespaceError) while sending active ops.", "RED")
        connected_to_server = False
    except Exception as e:
        logger.log_message(
            f"Error collecting or sending active operations: {e}", "RED")
...
def calculate_variation(price1, price2):
    ...
    try:
        ...
        logger.log_message(
            f"Error converting prices to float for variation calculation: {price1}, {price2}", "RED")
        return 0.0
...
def calculate_tp_sl(operation_type, current_price):
    ...
    try:
        ...
        logger.log_message(
            f"Error calculating TP/SL for price {current_price}", "RED")
        return 0.0, 0.0
...
def calculate_difference(entry_price, current_price, operation_type):
    ...
    try:
        ...
        logger.log_message(
            f"Error calculating difference: {entry_price}, {current_price}", "RED")
        return 0.0
...
def check_deactivation(operation_type, current_price, operation_data):
    ...
    if tp is None or sl is None:
        logger.log_message(
            f"Missing TP/SL in operation data for {operation_data.get('tick', 'N/A')}", "RED")
        return False, None
    try:
        price = float(current_price)
        win_or_lose = None
        ...
        if operation_type['name'] == config.LONG_NAME:
            if price >= tp:
                deactivate, win_or_lose = True, config.WIN_NAME
            elif price <= sl:
                deactivate, win_or_lose = True, config.LOSE_NAME
        else:
            if price <= tp:
                deactivate, win_or_lose = True, config.WIN_NAME
            elif price >= sl:
        ...
        logger.log_message(
            f"Error checking deactivation for price {current_price}", "RED")
        return False, None
    ...
    try:
        ...
        if variation >= var_perc:
            ...
            if info is None or 'quoteVolume' not in info:
                logger.log_message(
                    f"Could not get volume info for {tick} to check entry condition.", "RED")
                return
        ...
        logger.log_message(
            f"Error: Key '{e}' missing in config.TRADING_PARAMS (process_entry).", "RED")
    except (ValueError, TypeError) as e:
        logger.log_message(
            f"Data error when processing entry for {tick}: {e}", "RED")
    except Exception as e:
        logger.log_message(
            f"Unexpected error in process_entry_condition for {tick}: {e}", "RED")
...
def trigger_new_operation(tick, operation_type, current_price):
    ...
    try:
        ...
        active_ops_count = sum(
            1 for op in possible_operations.values() if op.get('is_active', False))
        if active_ops_count >= config.MAX_CONCURRENT_OPERATIONS:
            logger.log_message(
                f"Max concurrent operations ({config.MAX_CONCURRENT_OPERATIONS}) reached. Ignoring potential entry for {tick}.", "YELLOW")
            return

        tp, sl = calculate_tp_sl(operation_type, current_price)
        if tp == 0.0 and sl == 0.0:
            logger.log_message(
                f"Failed to calculate TP/SL for {tick}, cannot start operation.", "RED")
            return
        ...
        log_operation_start(config.LOG_PATH, config.PIN,
                            config.TRADING_PARAMS, operation_data)
        ...
        logger.log_message(
            f"Error accessing config or results key during operation trigger: {e}", "RED")
    except Exception as e:
        logger.log_message(
            f"Unexpected error triggering operation for {tick}: {e}", "RED")
...
def evaluate_variation_from_klines(tick, klines):
    ...
    try:
        ...
        if initial_price > final_price:
            ...
            process_entry_condition(
                tick, variation, config.LONG_NAME, final_price)
        elif final_price > initial_price:
            variation = calculate_variation(final_price, initial_price)
            process_entry_condition(
                tick, variation, config.SHORT_NAME, final_price)

        if len(klines) >= 3:
            prev_prev_price = float(klines[last_closed_candle_index - 2][4])
            current_price = final_price
            if prev_prev_price == 0:
                return
            if current_price > prev_prev_price:
                fast_variation = calculate_variation(
                    current_price, prev_prev_price)
                var_fast_perc = config.TRADING_PARAMS.get(
                    'VARIATION_FAST_PERCENTAGE', 1.0)
                if fast_variation >= var_fast_perc:
                    trigger_new_operation(
                        tick, config.TYPE_DEFINITIONS[config.FAST_SHORT_NAME], current_price)
    except IndexError:
        logger.log_message(
            f"Index error evaluating variation for {tick} (klines len: {len(klines)}).", "RED")
    except (ValueError, TypeError) as e:
        logger.log_message(
            f"Data error evaluating variation for {tick}: {e}", "RED")
    except Exception as e:
        logger.log_message(
            f"Unexpected error in evaluate_variation_from_klines for {tick}: {e}", "RED")
...
def evaluate_active_operations():
    ...
    active_ticks = [
        tick for tick, op in possible_operations.items() if op.get('is_active', False)]
    if not active_ticks:
        return
    ...
    for tick in active_ticks:
        ...
        try:
            ...
            if info is None or 'lastPrice' not in info:
                logger.log_message(
                    f"Could not get current price for {tick} during evaluation.", "RED")
                continue
            ...
            tp_str = f"{operation_data.get('tp', 'N/A'):.8f}" if isinstance(
                operation_data.get('tp'), float) else 'N/A'
            sl_str = f"{operation_data.get('sl', 'N/A'):.8f}" if isinstance(
                operation_data.get('sl'), float) else 'N/A'
            logger.log_message(
                f"Eval: {tick} ({operation_type['name']}) Diff: {difference:.2f}%", color)

            log_operation_progress(
                config.LOG_PATH, operation_data, current_price, difference)

            deactivate, final_status = check_deactivation(
                operation_type, current_price, operation_data)

            if deactivate:
                logger.log_message(f"Operation {tick} hit {final_status} at price {current_price:.8f} (Diff: {difference:.2f}%)",
                                   "GREEN" if final_status == config.WIN_NAME else "RED")
            ...
            logger.log_message(
                f"Data error evaluating operation {tick}: {e}", "RED")
        except Exception as e:
            logger.log_message(
                f"Unexpected error evaluating operation {tick}: {e}", "RED")

    # --- Finalize Operations ---
    if operations_to_finalize:
        logger.log_message(
            f"[Eval] Finalizing {len(operations_to_finalize)} operations.")
        ...
        if tick_to_finalize in possible_operations:
            ...
            try:
                ...
                if results[config.IN_PROGRESS_NAME][op_type['name']] > 0:
                    ...
                    logger.log_message(
                        f"Warning: In-progress count for {op_type['name']} was already 0 when finalizing {tick_to_finalize}.", "YELLOW")
            except KeyError:
                logger.log_message(
                    f"Error: Key not found updating results for {tick_to_finalize} ({op_type['name']}/{final_status}).", "RED")

            finalize_operation_log(
                config.LOG_PATH, op_data, final_status, item['final_price'], item['final_difference'])
        else:
            logger.log_message(
                f"Error: Tried to finalize operation {tick_to_finalize}, but it was not found in possible_operations.", "RED")
...
def save_aggregated_results():
    ...
    try:
        ...
        logger.log_message(
            f"Error calculating/saving aggregated results or sending updates: {e}", "RED")
    ...
    while True:
        try:
            if not binance_service.is_connected():
                logger.log_message(
                    "Scanner: Binance client not connected, skipping scan.", "RED")
            ...
            if not symbols:
                logger.log_message(
                    "Scanner: No USDT symbols found or error fetching.", "YELLOW")
            else:
                processed_count = 0
                for tick in symbols:
...
def evaluation_cycle():
    ...
    while True:
        try:
            ...
            if not binance_service.is_connected():
                logger.log_message(
                    "Evaluator: Binance client not connected, skipping evaluation.", "RED")
                continue
            evaluate_active_operations()  # This now handles sending updates if needed
        except Exception as e:
            logger.log_message(
                f"CRITICAL error in evaluation cycle: {e}", "RED")
...
def connect_to_socketio_server():
    ...
    original_log_message(
        f"Attempting to connect to Socket.IO server at {config.SERVER_URL}...")
    try:
        sio_client.connect(config.SERVER_URL, wait_timeout=10)
    except socketio.exceptions.ConnectionError as e:
        original_log_message(
            f"Could not connect to Socket.IO server: {e}. Web logs/stats unavailable.", "RED")
    except Exception as e:
        original_log_message(
            f"Unexpected error connecting to Socket.IO server: {e}", "RED")
...
def main():
    ...
    if not binance_service.is_connected():
        original_log_message(
            "CRITICAL: Binance client failed to initialize. Bot cannot start.", "RED")
        sys.exit(1)

    connect_to_socketio_server()

    logger.log_message(
        f'Starting Trading Bot Cycles... PIN: {config.PIN}', 'GREEN')
    ...
    try:
        while True:
            if not connected_to_server:
                original_log_message(
                    "Attempting to reconnect to Socket.IO server...")
                connect_to_socketio_server()
            time.sleep(60)
